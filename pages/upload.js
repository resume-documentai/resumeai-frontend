    ...
    const { getRootProps, getInputProps } = useDropzone({
        ...
        onDrop: async (acceptedFiles) => {
            ...
            try {
                ...
                setResumeFeedback("An error occurred while processing your file. Please try again.");
            ...
            <Layout>
                <div className="h-full flex flex-col items-center justify-center p-4" style={{minHeight: `calc(100vh - 56px - 128px)`}}>
                    <div className="p-6 bg-gray-300 rounded-lg shadow-lg w-96 text-center">
                        ...
                        <div 
                            {...getRootProps()} 
                            className={`border-2 border-dashed border-gray-400 p-6 cursor-pointer mb-4 ${isLoading ? 'opacity-50 cursor-not-allowed' : ''}`}
                        >
                            ...
                            <div className="flex flex-col items-center">
                                {isLoading ? (
                                    ...
                                    <p>Drag and Drop your resume, or click to select a file. (PDF or DOCX)</p>
                                )}
                            </div>
                        </div>
                    ...
                    {errorOccured ? <p className="text-red-500">An error occurred while processing your file. Please try again.</p> : <ResumeFeedback feedback={resumeFeedback} />}
                </div>
            </Layout>
        </ProtectedRoute>
    );
